#include <iostream>
#include <utility>
#include <vector>
#include <cmath>
#include <iomanip>



const double eps = 1E-6;
const double delta = 0.1;


using namespace std;

struct Elem{
    double i;
    Elem* next;

    Elem(){
        next = nullptr;
    }
    explicit Elem(double i){
        this -> i = i;
        next = nullptr;
    }
};


class Stack{

public:
    Elem* head;

    Stack(){
        head = nullptr;
    }


    double first() const{
        if(head != nullptr){
            return head -> i;
        }
        else{
            //cout << "Our struct is empty ";
            return 0;
        }
    }


    bool empty() const{
        return head == nullptr;
    }


    void pop(){
        if(head != nullptr){
            Elem* p = head;
            head = head -> next;
            delete p;
        }
    }


    void push(double i){
        Elem* el = new Elem(i);
        el -> next = head;
        head = el;
    }


    void print() const{
        Elem*temp = head;
        while(temp != nullptr){
            cout << temp -> i << ' ';
            temp = temp -> next;
        }
        cout << endl;
    }
};


double pow(double a, double n){
    double result = 1;
    int i = 0;
    while (i < n){
        result *= a;
        i++;
    }
    return result;
}

double sqrt(int x) {
    if (x==0) return 0;
    int left = 1;
    int right = x/2 + 1;
    double res;

    while (left <= right) {
        int mid = left + ((right-left)/2);
        if (mid<=x/mid){
            left = mid+1;
            res=mid;
        }
        else {
            right=mid-1;
        }
    }

    return res;
}


int prior(char v){
    switch(v){

        case '(': return 1;


        case '+':
        case '-': return 2;


        case '*':
        case '/': return 3;

        case '^': return 4;

        default:{
            //cout << "ERROR! Input correct data!";
            return 0;
        }
    }
}


bool if_operator(char c){
    return c == '+' || c == '-' || c == '*' || c == '/' || c == '^';
}


bool if_digit(char c){
    return (isdigit(c) != 0 || c == 'x' || c == 'y');
}


void OPN(char *a, char *out){
    Stack steck;
    int i = 0, j = 0;
    while (a[i] != '\0'){
        if (if_digit(a[i])){
            out[j] = a[i];
            j++;
        }


        else if (if_operator(a[i])) {
            if (steck.empty() || prior(steck.first()) < prior(a[i])) {
                steck.push(a[i]);
            } else {
                while (prior(steck.first()) >= prior(a[i]) && !steck.empty()) {
                    out[j++] = steck.first();
                    steck.pop();
                }
                steck.push(a[i]);
            }
        }

        else if (a[i] == '('){
            steck.push(a[i]);
        }
        else if (a[i] == ')'){
            if (steck.empty() || steck.first() == '('){
                //cout << "Input error!";
                exit (1);
            }
            else {
                while (steck.first() != '('){
                    out[j] = steck.first();
                    steck.pop();
                    j++;
                }
            }
            steck.pop();
        }

        i++;
    }

    while (!steck.empty()){
        if (steck.first() == '('){
            //cout << "Input error!";
            exit(1);
        }
        else{
            out[j] = steck.first();
            steck.pop();
            j++;
        }
    }
}



std::string* strparse(std::string& str){
    auto* parsestr = new std::string[30];
    int parse = 0;
    std::string save;
    for(int i = 0; i < str.size(); i++){
        if(isdigit(str[i])){
            int j = i;
            save += str[j];
            j++;
            while(isdigit(str[j])) {
                save += str[j];
                j++;
                i++;
            }
            parsestr[parse] = save;
            parse++;
            save.clear();
        }

        else {
            save += str[i];
            parsestr[parse] = save;
            parse++;
            save.clear();
        }


    }
    return parsestr;
}


double CalculateDeriative(std::string* str, double ans){

    double res;
    for(int i = 0; i < 30; i++){
        if(str[i] == "x"){
            str[i] = to_string(ans);
            if(str[i + 1] == "^") {
                double left = stod(str[i]);
                double right = stod(str[i + 2]);
                str[i] = to_string(double(pow(left, right)));
                str[i + 1].clear();
                str[i + 2].clear();
            }
        }
    }


    int j = 0;
    auto* str2 = new std::string[30];
    for(int i = 0; i < 30; i++){
        if(!str[i].empty()) {
            str2[j] = str[i];
            j++;
        }
    }


    for(int i = 0; i < str2->size() + 10; i++){
        if(str2[i + 1] == "*") {
            double left = stod(str2[i]);
            double right = stod(str2[i + 2]);
            str2[i] = to_string(double(left * right));
            str2[i + 1].clear();
            str2[i + 2].clear();
        }
    }




    j = 0;
    auto* str3 = new std::string[str2->size() + 10];
    for(int i = 0; i < str3->size() + 20; i++){
        if(!str2[i].empty()) {
            str3[j] = str2[i];
            j++;
        }
    }

    if(str3[1].empty()){
        return stod(str3[0]);
    }



    double one = stod(str3[0]);
    double two = stod(str3[2]);

    char symbol1 = str3[1][0];

    if(symbol1 == '+'){
        res = one + two;
    }
    else
        res = one - two;

    return res;
}


std::string Deriative(const char* input,char *out){
    Stack steck;
    std::string deriative;
    auto* strtwo = new std::string[10];
    int j = 0;
    char temp[] = {0};
    double head1, head2;
    double head3 = 0;
    bool trigger = false;
    while(out[j] != '\0'){

        if(out[j] == 'y'){
            out[j] = ' ';
            out[j - 1] = ' ';
            out[j - 2] = ' ';
            out[j + 1] = ' ';
            out[j + 1] = ' ';
        }



        if(if_digit(out[j]) && out[j] != 'x') {
            temp[0] = out[j];
            double temp1 = atof(temp);
            steck.push(temp1);
            if(strtwo[0].empty())
                strtwo[0] = out[j];
            else if(strtwo[1].empty() && !strtwo[0].empty())
                strtwo[1] = out[j];
            else if(strtwo[2].empty() && !strtwo[1].empty())
                strtwo[2] = out[j];
        }

        else if(out[j] == 'x') {
            if(strtwo[0].empty())
                strtwo[0] = out[j];
            else if(strtwo[1].empty() && !strtwo[0].empty())
                strtwo[1] = out[j];
            else if(strtwo[2].empty() && !strtwo[1].empty())
                strtwo[2] = out[j];
            else if(strtwo[3].empty() && !strtwo[2].empty())
                strtwo[3] = out[j];
            else if(strtwo[4].empty() && !strtwo[3].empty())
                strtwo[4] = out[j];
        }


        else if(if_operator(out[j])){
            head1 = steck.first();
            steck.pop();
            head2 = steck.first();
            steck.pop();

            std::string save;

            if(!steck.empty()) {
                char third = steck.first();
                steck.pop();
                save = (to_string(third) + to_string(head2));
                head3 = std::stod(save);
            }


            if(strtwo[0][0] != 'x' && strtwo[1][0] != 'x'  && head3 == 0){
                save = (to_string(int(head2)) + to_string(int(head1)));
                head3 = std::stod(save);
            }


            if(out[j] == '^') {
                if (head3 == 0) {
                    if (out[j - 1] == '3' && input[0] != '-')
                        deriative += (to_string(int(head1 * head2)) + "*x^" + to_string(int(head1 - 1)));
                    else if (out[j - 1] == '3' && input[0] == '-')
                        deriative += ("-" + to_string(int(head1 * head2)) + "*x^" + to_string(int(head1 - 1)));

                    else if (out[j - 1] == '2' && out[j + 2] == '+' && !deriative.empty())
                        deriative += ("+" + to_string(int(head1 * head2)) + "*x");
                    else if (out[j - 1] == '2' && out[j + 2] == '-' && !deriative.empty())
                        deriative += ("-" + to_string(int(head1 * head2)) + "*x");

                    else if (out[j - 1] == '2' && input[0] != '-' && deriative.empty()) {
                        deriative += (to_string(int(head1 * head2)) + "*x");
                    }
                    else if (out[j - 1] == '2' && input[0] == '-' && deriative.empty()) {
                        deriative += ("-" + to_string(int(head1 * head2)) + "*x");
                    }
                }


                else {
                    if (out[j - 1] == '3' && input[0] != '-')
                        deriative += (to_string(int(head3 * head1)) + "*x^" + to_string(int(head1 - 1)));
                    else if (out[j - 1] == '3' && input[0] == '-')
                        deriative += ("-" + to_string(int(head3 * head1)) + "*x^" + to_string(int(head1 - 1)));

                    else if (out[j - 1] == '2' && out[j + 2] == '+' && !deriative.empty())
                        deriative += ("+" + to_string(int(head3 * head1)) + "*x");
                    else if (out[j - 1] == '2' && out[j + 2] == '-' && !deriative.empty())
                        deriative += ("-" + to_string(int(head3 * head1)) + "*x");

                    else if (out[j - 1] == '2' && input[0] != '-' && deriative.empty()) {
                        deriative += (to_string(int(head3 * head1)) + "*x");
                        trigger = true;
                    } else if (out[j - 1] == '2' && input[0] == '-' && deriative.empty()) {
                        deriative += ("-" + to_string(int(head3 * head1)) + "*x");
                        trigger = true;
                    }
                }
            }


            if(head3 == 0) {
                if (out[j] == '*') {
                    if (out[j - 1] == 'x') {
                        if (out[j + 1] == '-')
                            deriative += ("-" + to_string(int(head1)));
                        else if (out[j + 1] == '+')
                            deriative += ("+" + to_string(int(head1)));
                    }
                }
            }
            else {
                if (!trigger) {
                    if (out[j] == '*') {
                        if (out[j - 1] == 'x') {
                            if (out[j + 1] == '-') {
                                deriative += ("-" + to_string(int(head3)));
                            }
                            else if (out[j + 1] == '+') {
                                deriative += ("+" + to_string(int(head3)));
                            }
                        }
                    }
                } else {
                    if (out[j] == '*') {
                        if (out[j - 1] == 'x') {
                            if (out[j + 1] == '-')
                                deriative += ("-" + to_string(int(head3)));
                            else if (out[j + 1] == '+')
                                deriative += ("+" + to_string(int(head3)));

                        }
                    }
                }
            }

            save.clear();
        }
        j++;
        head3 = 0;
        trigger = false;

    }



    return deriative;
}





























std::string* strparseY(std::string& str){
    auto* parsestr = new std::string[30];
    int parse = 0;
    std::string save;
    for(int i = 0; i < str.size(); i++){
        if(isdigit(str[i])){
            int j = i;
            save += str[j];
            j++;
            while(isdigit(str[j])) {
                save += str[j];
                j++;
                i++;
            }
            parsestr[parse] = save;
            parse++;
            save.clear();
        }

        else {
            save += str[i];
            parsestr[parse] = save;
            parse++;
            save.clear();
        }


    }
    return parsestr;
}


double CalculateDeriativeY(std::string* str, double ans){

    double res;
    for(int i = 0; i < 30; i++){
        if(str[i] == "y"){
            str[i] = to_string(ans);
            if(str[i + 1] == "^") {
                double left = stod(str[i]);
                double right = stod(str[i + 2]);
                str[i] = to_string(int(pow(left, right)));
                str[i + 1].clear();
                str[i + 2].clear();
            }
        }
    }


    int j = 0;
    auto* str2 = new std::string[30];
    for(int i = 0; i < 30; i++){
        if(!str[i].empty()) {
            str2[j] = str[i];
            j++;
        }
    }


    for(int i = 0; i < str2->size() + 10; i++){
        if(str2[i + 1] == "*") {
            double left = stod(str2[i]);
            double right = stod(str2[i + 2]);
            str2[i] = to_string(double(left * right));
            str2[i + 1].clear();
            str2[i + 2].clear();
        }
    }




    j = 0;
    auto* str3 = new std::string[str2->size() + 10];
    for(int i = 0; i < str3->size() + 20; i++){
        if(!str2[i].empty()) {
            str3[j] = str2[i];
            j++;
        }
    }

    if(str3[1].empty()){
        return stod(str3[0]);
    }



    double one = stod(str3[0]);
    double two = stod(str3[2]);

    char symbol1 = str3[1][0];

    if(symbol1 == '+'){
        res = one + two;
    }
    else
        res = one - two;

    return res;
}


std::string DeriativeY(const char* input,char *out){
    Stack steck;
    std::string deriative;
    auto* strtwo = new std::string[10];
    int j = 0;
    char temp[] = {0};
    double head1, head2;
    double head3 = 0;
    bool trigger = false;
    while(out[j] != '\0'){

        if(out[j] == 'x'){
            out[j] = ' ';
            out[j - 1] = ' ';
            out[j - 2] = ' ';
            out[j + 1] = ' ';
            out[j + 1] = ' ';
        }



        if(if_digit(out[j]) && out[j] != 'y') {
            temp[0] = out[j];
            double temp1 = atof(temp);
            steck.push(temp1);
            if(strtwo[0].empty())
                strtwo[0] = out[j];
            else if(strtwo[1].empty() && !strtwo[0].empty())
                strtwo[1] = out[j];
            else if(strtwo[2].empty() && !strtwo[1].empty())
                strtwo[2] = out[j];
        }

        else if(out[j] == 'y') {
            if(strtwo[0].empty())
                strtwo[0] = out[j];
            else if(strtwo[1].empty() && !strtwo[0].empty())
                strtwo[1] = out[j];
            else if(strtwo[2].empty() && !strtwo[1].empty())
                strtwo[2] = out[j];
            else if(strtwo[3].empty() && !strtwo[2].empty())
                strtwo[3] = out[j];
            else if(strtwo[4].empty() && !strtwo[3].empty())
                strtwo[4] = out[j];
        }


        else if(if_operator(out[j])){
            head1 = steck.first();
            steck.pop();
            head2 = steck.first();
            steck.pop();

            std::string save;

            if(!steck.empty()) {
                char third = steck.first();
                steck.pop();
                save = (to_string(third) + to_string(head2));
                head3 = std::stod(save);
            }


            if(out[j] == '^') {
                if (head3 == 0) {
                    if (out[j - 1] == '3' && input[0] != '-')
                        deriative += (to_string(int(head1 * head2)) + "*y^" + to_string(int(head1 - 1)));
                    else if (out[j - 1] == '3' && input[0] == '-')
                        deriative += ("-" + to_string(int(head1 * head2)) + "*y^" + to_string(int(head1 - 1)));

                    else if (out[j - 1] == '2' && out[j + 2] == '+' && !deriative.empty())
                        deriative += ("+" + to_string(int(head1 * head2)) + "*y");
                    else if (out[j - 1] == '2' && out[j + 2] == '-' && !deriative.empty())
                        deriative += ("-" + to_string(int(head1 * head2)) + "*y");

                    else if (out[j - 1] == '2' && input[0] != '-' && deriative.empty()) {
                        deriative += (to_string(int(head1 * head2)) + "*y");
                    }
                    else if (out[j - 1] == '2' && input[0] == '-' && deriative.empty()) {
                        deriative += ("-" + to_string(int(head1 * head2)) + "*y");
                    }
                }


                else {
                    if (out[j - 1] == '3' && input[0] != '-')
                        deriative += (to_string(int(head3 * head1)) + "*y^" + to_string(int(head1 - 1)));
                    else if (out[j - 1] == '3' && input[0] == '-')
                        deriative += ("-" + to_string(int(head3 * head1)) + "*y^" + to_string(int(head1 - 1)));

                    else if (out[j - 1] == '2' && out[j + 2] == '+' && !deriative.empty())
                        deriative += ("+" + to_string(int(head3 * head1)) + "*y");
                    else if (out[j - 1] == '2' && out[j + 2] == '-' && !deriative.empty())
                        deriative += ("-" + to_string(int(head3 * head1)) + "*y");

                    else if (out[j - 1] == '2' && input[0] != '-' && deriative.empty()) {
                        deriative += (to_string(int(head3 * head1)) + "*y");
                        trigger = true;
                    } else if (out[j - 1] == '2' && input[0] == '-' && deriative.empty()) {
                        deriative += ("-" + to_string(int(head3 * head1)) + "*y");
                        trigger = true;
                    }
                }
            }


            if(head3 == 0) {
                if (out[j] == '*') {
                    if (out[j - 1] == 'y') {
                        if (out[j + 1] == '-')
                            deriative += ("-" + to_string(int(head1)));
                        else if (out[j + 1] == '+')
                            deriative += ("+" + to_string(int(head1)));
                    }
                }
            }
            else {
                if (!trigger) {
                    if (out[j] == '*') {
                        if (out[j - 1] == 'y') {
                            if (out[j + 1] == '-') {
                                deriative += ("-" + to_string(int(head3)));
                            }
                            else if (out[j + 1] == '+') {
                                deriative += ("+" + to_string(int(head3)));
                            }
                        }
                    }
                } else {
                    if (out[j] == '*') {
                        if (out[j - 1] == 'y') {
                            if (out[j + 1] == '-')
                                deriative += ("-" + to_string(int(head3)));
                            else if (out[j + 1] == '+')
                                deriative += ("+" + to_string(int(head3)));

                        }
                    }
                }
            }

            save.clear();
        }
        j++;
        head3 = 0;
        trigger = false;

    }


    return deriative;
}



class Grad{
public:
    double x, y;
    double derx, dery;
    std::vector<double> Xi, Yi;
    std::string function;

    Grad(){
        x = y = 0;
        derx = dery = 0;
    }


    explicit Grad(std::string func){
        function = std::move(func);
        x = 8;
        y = -3;
        derx = dery = 0;
    }


    [[nodiscard]] double CalcDerX(double ans) const{
        char output[100];
        char input[100];
        for(int i = 0; i < 20; i++)
            input[i] = function[i];

        OPN(input, output);
        std::string funcs = Deriative(input, output);
        std::string* str = strparse(funcs);
        return CalculateDeriative(str, ans);
    }


    [[nodiscard]] double CalcDerY(double ans) const{
        char output[100];
        char input[100];
        for(int i = 0; i < 20; i++)
            input[i] = function[i];

        OPN(input, output);
        std::string funcs = DeriativeY(input, output);
        std::string* str = strparseY(funcs);
        return CalculateDeriativeY(str, ans);
    }


    void GradFunc(){
        Xi.push_back(x);
        Yi.push_back(y);
        derx = CalcDerX(Xi[0]);
        dery = CalcDerY(Yi[0]);
        int i = 1;
        Xi.push_back(Xi[0] - derx * delta);
        Yi.push_back(Yi[0] - dery * delta);
        derx = CalcDerX(Xi[i]);
        dery = CalcDerY(Yi[i]);
        while(sqrt(pow((Xi[i] - Xi[i - 1]), 2) + pow((Yi[i] - Yi[i - 1]), 2)) > eps){
            i++;
            Xi.push_back(Xi[i - 1] - derx * delta);
            Yi.push_back(Yi[i - 1] - dery * delta);
            derx = CalcDerX(Xi[i]);
            dery = CalcDerY(Yi[i]);
        }

        for(int j = 0; j < Xi.size(); j++){
            std::cout << std::fixed << setprecision(7) << Xi[j] << "\t\t" << Yi[j] << "\t\t" << "\n";
        }

    }
};


int main() {
    Grad grad("2*x^2+2*y^2");
    grad.GradFunc();
    return 0;
}
